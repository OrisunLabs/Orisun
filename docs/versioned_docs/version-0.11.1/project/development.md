...
```bash
./build.sh "$(go env GOOS)" "$(go env GOARCH)" "$(git describe --tags --always)" pg
./collect_benchmarks.sh --help
```

Run the generated server binary before starting the external benchmark. See
[Benchmarks](../operations/benchmarks.mdx) for the reproducible workload and
result format.
