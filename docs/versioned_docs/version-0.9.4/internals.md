...
5. Every server also establishes a process-unique
   ...
   local request gate. Local installation brings the physical schema up to the
   version this process requires before registering it in the backend SQL
   adapters. A boundary catalogued by an older Orisun release therefore has its
   initializer applied on the installing process even when that process is not
   the provisioning controller, so schema objects such as the index-metadata
   table exist before any adapter reads them. Installation reuses the migration
   already run by this process's controller path when the schema matches, so
   that process does not repeat the DDL during activation.
